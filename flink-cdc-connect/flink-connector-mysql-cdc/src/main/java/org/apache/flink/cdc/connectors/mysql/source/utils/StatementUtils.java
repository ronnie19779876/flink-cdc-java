...
public class StatementUtils {
    ...
    public static int queryActualRowCnt(JdbcConnection jdbc,
                                        TableId tableId,
                                        String columnName,
                                        Object[] minMaxOfSplitColumn) throws SQLException {
        final String rowCountQuery =
                String.format(
                        "SELECT COUNT(1) FROM %s WHERE %s BETWEEN ? AND ?",
                        quote(tableId), quote(columnName));
        Connection connection = jdbc.connection();
        ResultSet resultSet = null;
        PreparedStatement statement = null;
        try {
            statement = connection.prepareStatement(rowCountQuery);
            statement.setObject(1, minMaxOfSplitColumn[0]);
            statement.setObject(2, minMaxOfSplitColumn[1]);
